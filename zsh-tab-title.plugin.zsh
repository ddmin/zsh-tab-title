...
function omz_termsupport_precmd {
  ...
  ##################################
  # truncate to last two directories
  #     and replace $HOME with ~
  ##################################
  PWD_CURRENT=$PWD
  if [[ "$PWD_CURRENT" == *"$HOME"* ]]; then
      PWD_CURRENT=$(echo $PWD_CURRENT | sed "s|$HOME|~|g")
  fi

  ZSH_THEME_TERM_TAB_TITLE_IDLE="${PWD_CURRENT#"${PWD_CURRENT%/*/*}/"}"
...
# Execute the first time, so it shows correctly on terminal load
